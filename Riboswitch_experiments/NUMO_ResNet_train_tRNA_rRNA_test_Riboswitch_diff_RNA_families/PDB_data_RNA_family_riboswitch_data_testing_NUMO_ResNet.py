...
from data_preparation_RNA_mat_3d_nt_localized_info_mat_function_Riboswitch_v2 import data_prep_RNA_mat_3d_nt_localized_info_mat_riboswitch_data
import torch
import torch.nn as nn
from train_val_test_plot_functions import create_weighted_sampler, dataloader_prep_with_sampler, dataloader_prep, test_ResNet_expert, train_val_figure_plot_function
import numpy as np

def RNA_family_testing_function_riboswitch_data_NUMO_ResNet(test_data_file, riboswitch_data_set, device_utilized_para, loss_function_utilized_para, testing_data_name_para, NN_test_model_best_acc_para, best_val_acc_model_name_para, NN_test_model_best_loss_para, best_val_loss_model_name_para, NN_test_model_last_epoch_para, model_from_last_epoch_name_para):
    ...
    test_x_color_mat_np_RNA_families, test_x_nt_localized_info_mat_np_RNA_families, test_y_np_RNA_families = data_prep_RNA_mat_3d_nt_localized_info_mat_PDB_data(Original_RNA_Data = Original_RNA_Data_test, padding_length=410.0)
    test_x_color_mat_np_riboswitch_data, test_x_nt_localized_info_mat_np_riboswitch_data, test_y_np_riboswitch_data = data_prep_RNA_mat_3d_nt_localized_info_mat_riboswitch_data(Original_RNA_Data = riboswitch_data_set, padding_length=410.0)

    test_x_color_mat_np = np.vstack((test_x_color_mat_np_RNA_families, test_x_color_mat_np_riboswitch_data))
    test_x_nt_localized_info_mat_np = np.vstack((test_x_nt_localized_info_mat_np_RNA_families, test_x_nt_localized_info_mat_np_riboswitch_data))
    test_y_np = np.concatenate((test_y_np_RNA_families, test_y_np_riboswitch_data), axis=0)
