...
def ENTRNA_testing_function_train_tRNA_rRNA_test_other_RNA_families_riboswitch_data(testing_data_file_name, riboswitch_data):
    # 1. Load the data
    Original_RNA_Data_train = pd.read_csv(testing_data_file_name)
    Original_RNA_Data_train_riboswitch_data = riboswitch_data

    # 2. Select columns
    Original_RNA_Data_train = Original_RNA_Data_train[['RNA_name', 'RNA_seq', 'RNA_secondary_structure', 'RNA_generated_seq']]

    Original_RNA_Data_train_riboswitch_data = Original_RNA_Data_train_riboswitch_data[['RNA_name', 'RNA_seq', 'RNA_secondary_structure']]
    ...
    Original_RNA_Data_train_riboswitch_data['RNA_Label'] = 1 
    ...
    Original_RNA_Data_train_riboswitch_data = Original_RNA_Data_train_riboswitch_data.rename(columns={"RNA_seq":"RNA_sequence", "RNA_secondary_structure":"RNA_sec_structure"})

    # 7. Concatenate two data sets
    Original_RNA_Data_combined = pd.concat([Original_RNA_Data_actual_RNA_structure, Original_RNA_Data_predicted_RNA_structure, Original_RNA_Data_train_riboswitch_data], axis=0, ignore_index=True) 
